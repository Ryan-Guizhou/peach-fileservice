 ...
 * @Version 1.0.0
 * @Description //TODO
 * @CreateTime 2025/6/26 22:28
 */
@Slf4j
@Indexed
@Component
public class CustomSftpFactory extends BasePooledObjectFactory<SshChannel> {

    /**
     *  sftp 地址
     */
    @Value("${sftp.custom.host:47.121.112.66}")
    private String host;

    /**
     *  sftp 用户名
     */
    @Value("${sftp.custom.account:root}")
    private String account;

    /**
     * 密码
     */
    @Value("${sftp.custom.password:123456}")
    private String password;

    /**
     * 私钥 私钥地址
     */
    @Value("${sftp.custom.privateKey:C:/Users/Administrator/.ssh/id_rsa-pem}")
    private String privateKey;

    /**
     *
     */
    @Value("${sftp.custom.passphrase:624511}")
    private String passphrase;

    /**
     * 端口号
     */
    @Value("${sftp.custom.port:22}")
    private int port;
    ...
    public void destroyObject(PooledObject<SshChannel> pooledObject, DestroyMode destroyMode) throws Exception {
        ...
        try {
//            ChannelSftpUtils.destroyChannel(channel, ChannelSftpConnectionPools.nasChannelSftp);
        } catch (Exception e) {
